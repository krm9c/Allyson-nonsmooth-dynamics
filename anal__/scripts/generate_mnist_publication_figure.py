#!/usr/bin/env python3
"""
Generate Nature Communications style publication figure for MNIST classification results.
5-task experiment with 3 seeds - generates 6-panel figure with error bands.

Added by Claude: Publication-quality figure for JMLR paper (MNIST dataset).
"""
...
def get_mnist_pkl_paths():
    """Get PKL file paths for MNIST dataset."""
    data_dir = BASE_DIR / "mnist" / "results"
    ...
    for seed in range(3):
        ...
        c1_dir = f"mnist_condition1_baseline_run{seed}"
        c1_subdir = f"mnist_condition1_baseline_run{seed}_run{seed}"
        c1_file = f"classification_mnist_cnn_run{seed}_records.pkl"
        conditions['C1: Baseline'].append(data_dir / c1_dir / c1_subdir / c1_file)

        # Condition 2: Heuristics
        c2_dir = f"mnist_condition2_heuristics_run{seed}"
        c2_subdir = f"mnist_condition2_heuristics_run{seed}_run{seed}"
        c2_file = f"classification_mnist_cnn_run{seed}_records.pkl"
        conditions['C2: Heuristics'].append(data_dir / c2_dir / c2_subdir / c2_file)

        # Condition 3: Arch Search (AWB in subdir name)
        c3_dir = f"mnist_condition3_arch_no_transfer_run{seed}"
        c3_subdir = f"mnist_condition3_arch_no_transfer_run{seed}_awb_run{seed}"
        c3_file = f"classification_mnist_cnn_awb_run{seed}_records.pkl"
        conditions['C3: Arch Search'].append(data_dir / c3_dir / c3_subdir / c3_file)

        # Condition 4: AWB Full
        c4_dir = f"mnist_condition4_awb_full_run{seed}"
        c4_subdir = f"mnist_condition4_awb_full_run{seed}_run{seed}"
        c4_file = f"classification_mnist_cnn_awb_run{seed}_records.pkl"
...
def compute_cl_metrics(data):
    ...
    # BWT: Backward transfer (positive = improvement for accuracy)
    # BWT = (1/(T-1)) * sum(a_T,i - a_i,i) for i < T
    bwt_values = [matrix[-1, i] - matrix[i, i] for i in range(n_tasks - 1)]
    bwt = np.mean(bwt_values) if bwt_values else 0.0

    # Forgetting: Accuracy drop from best to final (positive = forgetting)
...
def main():
    """Generate publication figure for MNIST dataset."""
    print("="*70)
    print("GENERATING PUBLICATION FIGURE - MNIST (5 Tasks, 3 Seeds)")
    print("="*70)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # Get PKL paths
    conditions = get_mnist_pkl_paths()
    ...
    # Print summary
    print("\n" + "="*90)
    print("CONTINUAL LEARNING METRICS SUMMARY - MNIST (5 Tasks, 3 Seeds)")
    print("="*90)
    ...
    output_base = "mnist_publication_figure"
    ...
    if PAPER_FIG_DIR.exists():
        paper_name = "mnist_5task_3seed_results"
