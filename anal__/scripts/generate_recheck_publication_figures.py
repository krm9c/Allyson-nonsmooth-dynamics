#!/usr/bin/env python3
"""
Generate Nature Communications style publication figures for sine and sine_noise datasets.
Uses data from data_analysis_recheck folder.

Added by Claude: Publication-quality figures for JMLR paper (both datasets).
"""
...
# Base directory for recheck data
...
def get_pkl_paths(dataset_name):
    """Get PKL file paths for a dataset."""
    data_dir = BASE_DIR / dataset_name / "results"

    # Build conditions based on dataset name
    conditions = {
        'C1: Baseline': [],
        'C2: Heuristics': [],
        'C3: Arch Search': [],
    ...
    for seed in range(3):
        ...
        c1_dir = f"{dataset_name}_condition1_baseline_run{seed}"
        c1_subdir = f"{dataset_name}_condition1_baseline_run{seed}_run{seed}"
        c1_file = f"regression_sine_fcnn_run{seed}_records.pkl"
        conditions['C1: Baseline'].append(data_dir / c1_dir / c1_subdir / c1_file)

        # Condition 2: Heuristics
        c2_dir = f"{dataset_name}_condition2_heuristics_run{seed}"
        c2_subdir = f"{dataset_name}_condition2_heuristics_run{seed}_run{seed}"
        c2_file = f"regression_sine_fcnn_run{seed}_records.pkl"
        conditions['C2: Heuristics'].append(data_dir / c2_dir / c2_subdir / c2_file)

        # Condition 3: Arch Search (AWB in subdir name)
        c3_dir = f"{dataset_name}_condition3_arch_no_transfer_run{seed}"
        c3_subdir = f"{dataset_name}_condition3_arch_no_transfer_run{seed}_awb_run{seed}"
        c3_file = f"regression_sine_fcnn_awb_run{seed}_records.pkl"
        conditions['C3: Arch Search'].append(data_dir / c3_dir / c3_subdir / c3_file)

        # Condition 4: AWB Full
        c4_dir = f"{dataset_name}_condition4_awb_full_run{seed}"
        c4_subdir = f"{dataset_name}_condition4_awb_full_run{seed}_run{seed}"
        c4_file = f"regression_sine_fcnn_awb_run{seed}_records.pkl"
        conditions['C4: AWB Full'].append(data_dir / c4_dir / c4_subdir / c4_file)
...
def generate_publication_figure(dataset_name, title_suffix=""):
    """Generate publication figure for a dataset."""
    print("="*70)
    print(f"GENERATING PUBLICATION FIGURE - {dataset_name.upper()} (5 Tasks, 3 Seeds)")
    print("="*70)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # Get PKL paths for this dataset
    conditions = get_pkl_paths(dataset_name)
    ...
    for cond_name, pkl_files in conditions.items():
        ...
        for seed_idx, pkl_path in enumerate(pkl_files):
            if pkl_path.exists():
                ...
                print(f"    Seed {seed_idx}: File not found - {pkl_path}")
    ...
    # Print summary
    print("\n" + "="*90)
    print(f"CONTINUAL LEARNING METRICS SUMMARY - {dataset_name.upper()} (5 Tasks, 3 Seeds)")
    print("="*90)
    print(f"{'Condition':<18} {'Avg MSE (lower better)':<24} {'BWT (higher better)':<24} {'Forgetting (lower better)':<24}")
    print("-"*90)
    for cond_name in conditions.keys():
        if cond_name in agg_metrics:
            ...
            print(f"{cond_name:<18} {avg_mse:<24} {bwt:<24} {forg:<24}")
    print("="*90)
    ...
    ax.set_ylim(0, max(means) * 1.35 if means else 0.1)
    ...
    output_base = f"{dataset_name}_publication_figure"
    ...
    if PAPER_FIG_DIR.exists():
        paper_name = f"{dataset_name}_5task_3seed_results"
        fig.savefig(PAPER_FIG_DIR / f'{paper_name}.png', dpi=300, facecolor='white')
        fig.savefig(PAPER_FIG_DIR / f'{paper_name}.pdf', facecolor='white')
...
def main():
    """Generate publication figures for both sine and sine_noise datasets."""
    parser = argparse.ArgumentParser(description='Generate publication figures for sine/sine_noise datasets')
    parser.add_argument('--dataset', type=str, choices=['sine', 'sine_noise', 'both'], default='both',
                        help='Dataset to generate figures for (default: both)')
    args = parser.parse_args()

    datasets = []
    if args.dataset == 'both':
        datasets = ['sine', 'sine_noise']
    else:
    ...
    for dataset in datasets:
        ...
        print(f"{'#'*80}\n")
        all_metrics[dataset] = generate_publication_figure(dataset)

    # Print comparison summary if both datasets processed
    if len(datasets) == 2:
        print("\n\n" + "="*100)
        print("COMPARISON SUMMARY: SINE vs SINE_NOISE")
        print("="*100)
        print(f"{'Condition':<18} {'SINE Avg MSE':<20} {'SINE_NOISE Avg MSE':<20} {'Difference':<15}")
        print("-"*100)
        for cond in ['C1: Baseline', 'C2: Heuristics', 'C3: Arch Search', 'C4: AWB Full']:
            if cond in all_metrics.get('sine', {}) and cond in all_metrics.get('sine_noise', {}):
                sine_mse = all_metrics['sine'][cond]['Avg_MSE_mean']
                noise_mse = all_metrics['sine_noise'][cond]['Avg_MSE_mean']
                diff = noise_mse - sine_mse
                print(f"{cond:<18} {sine_mse:<20.4f} {noise_mse:<20.4f} {diff:+.4f}")
        print("="*100)
