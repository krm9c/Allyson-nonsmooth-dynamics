...
\title[The Effect of Architecture During Continual Learning]{The Effect of Architecture During Continual Learning}
...
\date{September 4, 2025} % Date, can be changed to a custom date
...
%------------------------------------------------
    ...
    \begin{frame}{What is Continual Learning?}
        ...
        %\pause
        \begin{itemize}
            \item Let $t$ be a task instance in the set $\{0,1,\ldots, T\}\subset \mathbb{N}.$%\pause
            \item Let $\boldsymbol{\mathcal{X}}(t)$ represent the data set provided at each task $t.$%\pause
            \item The set $\boldsymbol{\mathcal{X}}(t)$ consists of a list of matrices/vectors/graphs. 
            %sampled according to the distribution $\mathbb{P}$ where $\boldsymbol{\mathcal{X}}(t) \subset \mathcal{D}$ such that $\mathcal{D}$ -- the domain, is a measurable set with a non-empty interior.
    ...
    \begin{frame}{Anomaly Detection on workflows}
  \centering \includegraphics[width = 0.45\columnwidth]{Figures/Picture1.png}
  \begin{block}{}
     \begin{itemize}
        \item Detect anomalies for workflows generated during scientific experiments
        \item We consider five workflows : Two unclustered workflows, Three clustered workflows
        \item Three anomalies are manually injected to collect data: loss, hdd, cpu
     \end{itemize}  
  \end{block}
  \centering {\color{WildStrawberry} The goal is to train a model to detect anomalies.}
  %\pause
  \begin{block}{\centering}
        \centering
        All this data is represented by graphs, therefore, let us think {\color{WildStrawberry}graph neural networks}.
    \end{block}
\end{frame}

% \begin{frame}{Anomaly Detection}
%     \centering \includegraphics[width =0.7\columnwidth]{Picture1.png}
%     \begin{block}{\centering}
%         \centering
%         All this data is represented by graphs, therefore, let us think {\color{WildStrawberry}graph neural networks}.
%     \end{block}
...
% \end{frame}
    ...
    \begin{frame}{Neural Networks for Continual Learning}
    \textbf{Question:} How do we train neural networks in the continual learning setting?%\pause
    \begin{figure}
        \centering
        %\includegraphics[width =0.75\textwidth]{Figures/W2.pdf} 
        \includegraphics[width =0.75\textwidth]{Figures/Wball1new2png.png} 
        \caption{Figure: The learning problem for task 1 (i.e. one giant dataset)}
    ...
    \begin{frame}{Neural Networks for Continual Learning}
        \centering
        \includegraphics[width =0.75\textwidth]{Figures/Wball2new1.png} \\
        \centering Figure: The learning problem for tasks 1 and 2.
    ...
    \begin{frame}{Neural Networks for Continual Learning}
        \centering
        \includegraphics[width =.7\textwidth]{Figures/Wball3new1.png} \\
        \centering Figure: The learning problem for tasks 1, 2, and 3.
    ...
    \begin{frame}{Capacity of the Intersection Space}
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment

        \column{.56\textwidth} % Left column and width
        \includegraphics[width =1\textwidth]{Figures/Wball3new1.png} \\
        \centering Figure: The learning problem for tasks 1, 2, and 3.
        ...
        \begin{itemize}
            ...
            \item The size of the intersection space depends on how similar or dissimilar task data is.%\pause
            
            \item The more tasks we learn, the \textbf{smaller} the size of the intersection space will become.\footfullcite{chakraborty2025understanding}
        \end{itemize}
    ...
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
        ...
        \begin{alertblock}{Problem}
            What if task data $\boldsymbol{\mathcal{X}}(t)$ is dissimilar to task data $\boldsymbol{\mathcal{X}}(t+1),$ resulting in an empty weights search spaces?
        \end{alertblock}
        ...
        \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figures/Wball3new1.png}
    ...
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
        ...
        \textbf{Theoretical:}
        \begin{itemize}
            \item No current way to measure the effect of task data on loss value%\pause
            \item Utilize Sobolev spaces and measure theory%\pause
            \item Prove: similar tasks imply similar loss values%\pause
            \item Prove: changing the architecture increases the capacity of the intersection space%\pause
        \end{itemize}

        \textbf{Experimental:}
        \begin{itemize}
            \item Develop algorithm which learns optimal architecture and weights%\pause
    ...
    Formulate the class of functions the neural network may represent as a Sobolev space\footfullcite{mahanNonclosednessSetsNeural2021a}.%\pause
    \vspace{.1 in}
    \begin{block}{}
    \centering\large\textbf{Why?}
    %\vspace{.1 in}
    \begin{itemize}
        \item Neural networks can be represented as a function of weights and architecture.%\pause
        \item Architecture is a discrete parameter. Hence, derivatives cannot be taken with respect to architecture in the classical (Euler) sense.%\pause
...
%\pause
\begin{itemize}
    \item Weak derivatives agree with pointwise derivative, where it exists.%\pause
    \item Weak derivatives are defined using integrals (integration by parts).%\pause
...
\begin{frame}{Target Function}
    ...
    \begin{definition}
        Let $f(t)\in W^{k,p}(\mathcal{D})$ for all $t\in \{0,\ldots, T\}$ denote the \textit{target function} which is to be approximated. If $\hat{f}(w(t),\psi(t))$ denotes a neural network determined to approximate $f(t)$ at task $t,$ then the \textit{loss function} (or error of approximation) for $x\in \boldsymbol{\mathcal{X}}(t)$ is given by 
        \begin{align*}
...
\begin{frame}{Basic Continual Learning Problem with New Machinery}
    \begin{figure}
    \centering
...
\begin{frame}{Basic Continual Learning Problem with New Machinery}
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
        ...
        \begin{itemize}
            ...
            % \item \textbf{Problem:} The intersection of search spaces is not required to be non-empty.
        \end{itemize}
        
        \column{.63\textwidth} % Right column and width
        \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figures/problem1new.png}
...
\begin{frame}{Revisiting The Problem We Investigate}
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
        ...
        \begin{itemize}
            ...
            \item \textbf{Problem:} The intersection of search spaces is not required to be non-empty.
            \vspace{2 in}
            % \item \textbf{What we need to do? :} Investigate how similar/dissimilar tasks impact the size of the intersection space.

            % \item \textbf{How do we do this? :} Utilize measure theory to gain a relationship between "size" of the intersection spaces average loss value.
        \end{itemize}
...
\begin{frame}{Revisiting The Problem We Investigate}
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
        ...
        \begin{itemize}
            ...
            \item \textbf{How do we do this?:} Utilize measure theory to gain a relationship between task similarity/dissimilarity and average loss values.
        \end{itemize}
...
\begin{frame}{Similar Tasks Imply Similar Loss}
 ...
 \label{defintersect}
     ...
     whenever $A,B\in \mathcal{B}(\overline{\boldsymbol{\mathcal{X}}})$ such that $\mu(A\bigtriangleup B)<\delta.$
 \end{definition}
\end{frame}
...
\begin{frame}{Measure of Set Symmetric Differences}
    \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{Figures/elementsincommon1.png}
        \label{fig:placeholder}
    \end{figure}
\end{frame}
\begin{frame}{Similar Tasks Imply Similar Loss}
    %Although one may logically deduce that the presence of dissimilar tasks leads to an empty intersection (and hence an empty search space), we must in fact prove this is true. To do so, we employ measure theory.
     \begin{block}{Definition 5 (Continuity with Respect to Measure)}
 \label{defintersect}
     Let $\overline{\boldsymbol{\mathcal{X}}} = \bigcup_{t=0}^\infty \boldsymbol{\mathcal{X}}(t)$ with the power set $\mathcal{P}(\overline{\boldsymbol{\mathcal{X}}})$ as its topology. Set $\mathcal{B}(\overline{\boldsymbol{\mathcal{X}}})$ to be the Borel $\sigma$-algebra on $\overline{\boldsymbol{\mathcal{X}}}$ equipped with a probability measure denoted $\mu.$ Then, $(\overline{\boldsymbol{\mathcal{X}}},\mathcal{B}(\overline{\boldsymbol{\mathcal{X}}}), \mu)$ forms a probability measure space. Let $g:\overline{\boldsymbol{\mathcal{X}}}\rightarrow \mathbb{R}$ be a measurable function, and set $F:\mathcal{B}(\overline{\boldsymbol{\mathcal{X}}})\rightarrow \mathbb{R}$ to be the function
     \begin{align*}
         F(A) & = \int_A g(x)d\mu.
...
%\pause
 \begin{exampleblock}{}
 \begin{center}
    ``Similar Sets" implies ``Similar Values of $F$"
 \end{center}
 \end{exampleblock}%\pause
 \textbf{Question:} How to we connect this to tasks and average loss value?
\end{frame}

\begin{frame}{Similar Tasks Imply Similar Loss}
    Recall that the loss function $\ell$ describes how well a NN $\fhat$ approximates the target function.

    We can define the expected value function (average loss function) for a set $A\in \mathcal{B}(\overline{\boldsymbol{\mathcal{X}}})$ to be
...
%     By disjoint additivity\footfullcite{weaver2013measure} and triangle inequality, 
%     \begin{align*}
...
%     \end{align*}
%     Thus, by disjoint additivity\footfullcite{weaver2013measure} of $\mu$ we have
%     \begin{align*}
...
\begin{frame}{What have we shown?}
    ...
    \begin{block}{}
    \begin{center}
        Similar Tasks $\Longrightarrow$ Similar Average Loss Values%\pause\\
        \textbf{and}\\
...
%Finding a tight lower bound requires us to consider the impact of the weights $w(t)$ and task data have on the average loss.
    ...
    \begin{align}
        \lim_{\Delta t\rightarrow 0} \frac{\abs{E(\boldsymbol{\mathcal{X}}(t)))-E(\boldsymbol{\mathcal{X}}(t+\Delta t))}}{\Delta t} &\geq \textcolor{red}{M_0\delta} - \textcolor{blue}{L_1\abs{\Delta w}}\textcolor{blue}{\displaystyle\int_{\boldsymbol{\mathcal{X}}(t)} \|\partial^1_w \hat{f}(w(t),\psi(1))\|_{L^p}d\mu}
    \end{align}
\end{theorem}
%\pause
\begin{itemize}
...
Notice, we gain a similar lower bound, but with an additional term for subtraction.
    ...
    \begin{align*}
        \|\nabla E(\boldsymbol{\mathcal{X}}(t))\| &\geq \textcolor{red}{M_0\delta} \textcolor{blue}{- L_1\abs{\Delta w}\displaystyle\int_{\boldsymbol{\mathcal{X}}(t)} \|\partial^1_w \hat{f}(w(t,\psi),\psi(t))\|_{L^p}d\mu}\\
        & \hspace{5mm}- \textcolor{Green4}{L_1\abs{\Delta \psi}\displaystyle\int_{\boldsymbol{\mathcal{X}}(t)} \|\partial^1_\psi \hat{f}(w(t,\psi),\psi(t))\|_{L^p}d\mu}.
    \end{align*}
\end{theorem}
\end{frame}
...
\begin{frame}{Adjusting the Lower Bound}
    As $\abs{\bigtriangleup \psi(t)}\geq 0$ and $L_1>0,$ it follows that
    \begin{align}
    ...
    \end{align}
    %\pause
    Thus, if $\abs{E(\boldsymbol{\mathcal{X}}(t))-E(\boldsymbol{\mathcal{X}}(t+\Delta t))}\geq \varepsilon$ by changing the architecture we can reduce $\|\nabla E(\boldsymbol{\mathcal{X}}(t))\|$ allowing the potential for
    \begin{align*}
...
\begin{frame}{Increasing the Intersection Space}
    \begin{figure}
        \centering
        \includegraphics[width=.75\linewidth]{Figures/archcompare1.png}
...
\begin{frame}{Increasing the Intersection Space}
    \begin{figure}
        \centering
        \includegraphics[width=.75\linewidth]{Figures/archcompare2.png}
        %\caption{Understanding the impact architecture has on the cardinality of the intersection search space.}
        \label{fig:enter-label}
    \end{figure}
\end{frame}
...
\section{Implementing The Theory: An Overview}
...
\begin{frame}{CL NN Learning Problem: Upper}
    ...
    \begin{align*}
        J(w(t),\psi,\boldsymbol{\mathcal{X}}(t)) & = \int_0^t \ell(\hat{f}(w(t),\psi(t))(X(\tau)))d\tau.
    \end{align*}
\end{frame}
...
\begin{frame}{Implementation of Upper Optimization: Architecture Search}
    \begin{figure}
        ...
        \label{fig:enter-label}
    \end{figure}\footfullcite{larson2019derivative}
\end{frame}
...
\begin{frame}{Implementation of Lower Optimization: HJB}
    Recall that for $V(t,w(t)) = \int_{t}^T J(w(\tau),\psi^*(t), \boldsymbol{\mathcal{X}}(\tau))d\tau$ where $\psi^*(t)$ is the solution to our upper (architecture) optimization, we had the lower optimization problem as
        \begin{align*}
            V^*(t,w(t)) & = \min_{w\in \mathcal{W}(\psi^*(t))} V(t,w(t)).
        \end{align*}%\pause
        \begin{exampleblock}{}
...
\begin{proposition}[Hamilton-Jacobi-Bellman Equation]
    ...
    \begin{align*}
        -\frac{\partial V^*}{\partial t} & = \min_{w\in \mathcal{W}(\psi^*(t))} J(w(t),\psi^*(t),\boldsymbol{\mathcal{X}}(t)) +\textcolor{Green4}{\frac{\partial V^*}{\partial \boldsymbol{\mathcal{X}}}\frac{d\boldsymbol{\mathcal{X}}}{dt}}+ \textcolor{blue}{\frac{\partial V^*}{\partial w}\frac{\partial w}{\partial t}} + \textcolor{red}{\frac{\partial V^*}{\partial \psi}\frac{\partial \psi}{\partial t}}.
    \end{align*}
\end{proposition}
...
\begin{frame}{Transfer of Learning to New Architecture: Low Rank Transfer}
    ...
    \begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
        ...
        \begin{itemize}
            \item \textbf{Question:} How do we transfer learning to a new architecture in practice?%\pause
...
\begin{proposition}
    ...
    \begin{align*}
        ...
        & \hspace{13mm} + \frac{\partial V^*}{\partial \boldsymbol{\mathcal{X}}}\frac{d\boldsymbol{\mathcal{X}}}{dt} + \textcolor{Green4}{\frac{\partial V^*}{\partial w}[A^*(t)w(t)(B^*(t))^T + u(t)]},
    \end{align*}
...
\begin{frame}{Regression Experiment 1: Set-Up}
    \begin{itemize}
        \item \textbf{Model:} MLP
        \item \textbf{Data set utilized:} Sine
        \item \textbf{Number of tasks learned:} $5$
        \item \textbf{Number of epochs per task:} $500$%\pause
        \item \textbf{Results:} displays recorded loss value with respect to epoch%\pause
        \begin{itemize}
            \item \textcolor{red}{Red:} Standard continual learning training on tasks $0$ through $4$ \footfullcite{raghavan2021formalizing}%\pause
            \item \textcolor{blue}{Blue:} Changing architecture at tasks $1$ through $4,$ but randomly re-initializing the weights (no Low Rank Transfer)%\pause
            \item  \textcolor{Green4}{Green:} Change the architecture at tasks $1$ through $4$ and utilize our $AWB^T$ method (with Low Rank Transfer)
        \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Regression Experiment 1: Changing Architecture on Every Task}
    \begin{figure}
        \centering
...
\begin{frame}{Regression Experiment 1: Changing Architecture on Every Task}
    \begin{figure}
        \centering
        \includegraphics[width=.93\linewidth]{Figures/origregtest.png}
    \end{figure}
\end{frame}

\begin{frame}{Regression Experiment 2: Set-Up}
    \begin{itemize}
        \item \textbf{Model:} MLP
        \item \textbf{Data set utilized:} Sine - with increased noise
        \item \textbf{Number of tasks learned:} $5$
        \item \textbf{Number of epochs per task:} $500$
        \item \textbf{Results:} the following graph displays recorded loss value with respect to epoch
        \begin{itemize}
            \item \textcolor{red}{Red:} Standard continual learning training on tasks $0$ through $4$ \footfullcite{raghavan2021formalizing}
            \item  \textcolor{blue}{Blue:} Change the architecture at tasks $1$ through $4$ and utilize our $AWB^T$ method (with Low Rank Transfer)
        \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}{Regression Experiment 2: Changing Architecture with Noisy Data}
    \begin{figure}
        \centering
        \includegraphics[width=.93\linewidth]{Figures/noisydata1.png}
    \end{figure}
\end{frame}

\begin{frame}{Regression Experiment 3: Set-Up}
    \begin{itemize}
    \item \textbf{Model:} MLP
        \item \textbf{Data set utilized:} Sine
        \item \textbf{Number of tasks learned:} $10$
        \item \textbf{Number of epochs per task:} $500$%\pause
        \item \textbf{Results:} the following graph displays recorded loss value with respect to epoch%\pause
        \begin{itemize}
            \item \textcolor{red}{Red:} Standard continual learning training on tasks $0$ through $9$ \footfullcite{raghavan2021formalizing}%\pause

            \item  \textcolor{blue}{Blue:} Change the architecture at tasks $1$ through $9$ and utilize our $AWB^T$ method (with Low Rank Transfer)
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Regression Experiment 3: Changing Architecture on Every Task}
    \begin{figure}
        \centering
        \includegraphics[width=.93\linewidth]{Figures/reg10tasks.png}
    \end{figure}
\end{frame}
...
\begin{frame}{Regression Experiment 3: Changing Architecture on Every Task}
    \begin{figure}
        \centering
        \includegraphics[width=.93\linewidth]{Figures/reg10Test.png}
    \end{figure}
\end{frame}

\begin{frame}{Classification Experiment 1: Set-Up}
    \begin{itemize}
    \item \textbf{Model:} CNN
        \item \textbf{Data set utilized:} Omniglot
        \item \textbf{Number of tasks learned:} $5$
        \item \textbf{Number of epochs per task:} $500$%\pause
        \item \textbf{Results:} the following graph displays recorded loss value with respect to epoch%\pause
        \begin{itemize}
...
\begin{frame}{Classification Experiment: Changing Architecture on Every Task}
    \begin{figure}
        \centering
        \includegraphics[width=1\linewidth]{Figures/cnn5tasks1.png}
    \end{figure}
\end{frame}


\begin{frame}{Graph Classification Experiment 1: Set-Up}
    \begin{itemize}
        \item \textbf{Model:} GCN
        \item \textbf{Data set utilized:} Synthetic
        \item \textbf{Number of tasks learned:} $5$
        \item \textbf{Number of epochs per task:} $125$%\pause
        \item \textbf{Results:} the following graph displays recorded loss value with respect to epoch%\pause
        \begin{itemize}
            \item \textcolor{red}{Red:} Standard continual learning training on tasks $0$ through $4$ \footfullcite{raghavan2021formalizing}%\pause
            \item \textcolor{blue}{Blue:} Changing architecture at tasks $1$ through $4,$ but randomly re-initializing the weights (no Low Rank Transfer)%\pause
...
\begin{frame}{Graph Classification Experiment: Changing Architecture on Every Task}
    \begin{figure}
        \centering
        \includegraphics[width=.9\linewidth]{Figures/gnnfix.png}
    \end{figure}
\end{frame}
