...
\section{Understanding the dynamics of tasks and architecture}
For illustration, we reconsider the example from section~\ref{sec:motivation} and draw Figure ~\ref{fig:challenge} where three tasks are shown to the model in order, with a fixed architecture, denoted by $\psi(1).$ \Ally{Small note. Do we want to use "Fig." or "Figure" for referencing figures? I don't have a preference, just wanted to note for consistency.} The goal is to find the optimal weights for the NN at each task thus solving~\eqref{eq:CL_for}. However, determining the optimal weights is highly dependent on each task's data. Intuitively, if the data in task $\tx(t) = \tx(t+1), \forall \; t \in [0,T],$ we can find a NN $\hat{f}$ which learns the new task and transfers learning from the previous tasks perfectly. But, if the$\tx(t) \ne \tx(t+1), \forall \; t \in [0,T],$ we may or may not be able to find a NN solution. 
\begin{figure}[!thb]
    \centering
    ...
    \label{fig:challenge}
\end{figure}
To illustrate this further, consider Figure  \ref{fig:challenge} and let $\mathcal{F}_t = \{\fhat(\weight,\psi)\in \sobolev\}$ to represent the search space for the set of all possible NN solutions for learning task $t$~(refer to  \ref{sec:motivation} for a similar construction). For illustration,  $\mathcal{F}_t$ is a subset of $\sobolev$ and can be thought of as a representative of a level set where the loss is less than a threshold. Pictorially, we represent this level set with a ball. At $t=1$, there is one ball, at task $t=2$ there are two balls, and at task $t=3$ there are three balls. The number of balls here correspond to the number of tasks being involved in the learning process at each step of learning. 
 
In each of the balls, \ding{58} represents the NN solution for only that particular task $t$ and the smileys face represents the solution that transfers learning among all tasks less than $t.$ Notably,  $\smiley{}$ must lie in the intersection of all the balls, as only in this intersection region do we have a solution that is optimal for all tasks. Curiously, unless the tasks as equivalent, the $\smiley{}$ and the \ding{58} will not coincide because no solution that is perfect for one task ends up being perfect for another. Furthermore, the more different the subsequent tasks are, it is intuitive that the intersection space is unlikely to be non-empty. To formalize this intuition, we first define the notion of the intersection space in $\sobolev$ in terms of its impact on the loss functions. To define this notion, we must first identify the notion of continuity in the space of continual learning loss function as a consequence of the difference in the data space. The definition of continuous functions w.r.t a measure is given as
     \begin{definition}
 \label{defintersect}
 ...
 \end{definition}
Definition \ref{defintersect} can be deduced from basic measure theory results found in \cite{weaver2013measure}. The intuition behind this definition is that sets which are similar~(with respect to the probability measure $\mu$) will result in integrals of $\fhat$ over the respective sets which are close in value. In the set theory context, the notation $A\bigtriangleup B$ represents set symmetric difference, which describes the elements two sets do not share~(see Figure  \ref{fig:setsim} for an illustration). Thus, as $\mu(A\bigtriangleup B)$ gets larger, the size of the intersection space (i.e. $\mu(A\cap B)$) shrinks. Hence, the measure of the set symmetric difference provides a notion of how ``similar" or how ``different" two sets are. In other words, we can acquire a notion of how similar or different two tasks are.
\begin{figure}
    \centering
...
\end{figure}
 ...
 In particular, the expected value of the loss function $\ell$ is continuous with respect to measure if for a small $\varepsilon >0$  we can find a $\delta >0$ such that $\abs{E(\tx(t))-E(\tx(t+1))}<\varepsilon$ for all $\tx(t+1)$ such that $\mu(\tx(t)\bigtriangleup \tx(t+1))<\delta.$ Thus, if two tasks $\tx(t)$ and $\tx(t+\Delta t)$ are similar (i.e. $\mu(\tx(t)\bigtriangleup \tx(t+\Delta t))<\delta)$, then the loss values are similar (i.e. $\abs{E(\tx(t))-E(\tx(t+\Delta t))}<\varepsilon$). This intuition is formalized in the following theorem, where we prove the expected value function is continuous with respect to measure.

%theorem proving EV is cont. w.r.t. meausre
\begin{theorem}\label{contMeasure}
    Let $(\overline{\tx},\mathcal{B}(\overline{\tx}), \mu)$ be the measure space as defined as in definition~\ref{defintersect}. Assume that the loss function $\ell$ is continuous and bounded across all tasks, that is $\forall t \in [0,T].$ Define an expected value of the loss function as 
    \begin{align}
        E(\tx(\tau)) = \int_{\vx \in \tx(\tau)} \ell(\fhat(\weight(t),\psi(t))( \vx ) )
    \end{align}
    Then, $ E(\tx(\tau))$ is continuous with respect to the measure for any $ \tx(\tau) \in \mathcal{B}(\overline{\tx}).$
\end{theorem}
\begin{proof}
    Suppose that the constant $M_0>0$ is the value which bounds $\ell$ on every task. Let $\varepsilon>0$ and  set $\delta = \varepsilon/M_0.$ Further let $A,B \in \mathcal{\mathcal{B}}(\overline{\tx})$ such that $\mu(A\bigtriangleup B)< \delta.$ By disjoint additivity of $\mu$ and triangle inequality, 
    \begin{align*}
        \abs{E(A) -E(B)} & = \Big\vert\int_{A} \ell(\fhat(\vx))d\mu- \int_B\ell(\fhat(\vx))d\mu\Big\vert\\
        & = \Big\vert\int_{A\setminus B} \ell(\fhat(\vx))d\mu+ \int_{A\cap B} \ell(\fhat(\vx))d\mu\\
        & \hspace{10mm}-\int_{B\setminus A}\ell(\fhat(\vx))d\mu-\int_{A\cap B}\ell(\fhat(\vx))d\mu\Big\vert\\
        & \leq \int_{A\setminus B} \abs{\ell(\fhat(\vx))}d\mu+\int_{B\setminus A}\abs{\ell(\fhat(\vx))}d\mu.
    \end{align*}
    Then, by the boundedness of $\ell,$
    \begin{align*}
        \int_{A\setminus B} \abs{\ell(\fhat(\vx))}d\mu+\int_{B\setminus A}\abs{\ell(\fhat(\vx))}d\mu & 
    ...
    \begin{align*}
        ...
        & < M_0 \delta\\
         ...
         & = \varepsilon.
    \end{align*}
    Then, choosing $A = \tx(t)$ and $B = \tx(t+1)$ provides the result.
\end{proof}
