...
\textbf{Datasets:} We utilize three data sets, one for each experiment.
...
\begin{itemize}
    ...
    \item \textbf{MNIST:}~\cite{lecun1998mnist} We split MNIST into sequential tasks by grouping digit pairs. Each task contains two digit classes. For example, Task $0$ consists of digits $0$ and $1,$ and Task 1 consists of digits $2$ and $3.$ Moreover, images are $28 \times 28$ grayscale.
...
\subsection{MNIST Classification Experiments}
...
\subsubsection{Mnist-ten tasks}
As an extension to Figure.~\ref{fig:mnist_comprehensive}, we conduct an experiment where we run the training from Figure.~\ref{fig:mnist_comprehensive} for 10 tasks and plot the results in Figure.~\label{cnn}. We note now, that instead of the blanket improvement we observed in the sine dataset, we observe a significant improvement at the beginning for the first three tasks and then there is a significant jump that leads to significant deterioration. This jump introduces the sharp change what was shown in our theory to deteriorate performance. 

On careful investigation it is observed from Figure.~\ref{fig:diagnostics} that Task 3 introduces a problematic behavior with a sudden and sharp change into the tasks leading to the models to perform poorly. The Proposed A/B training is expected to smooth over this change and this behavior is not observed in Figure.~\ref{fig:mnist_comprehensive}. Upon, careful investigation, it was observed that, the smoothing behavior is proportional to how many epochs are utilized for a/b training. 
...
Towards identifying the effect of A/B epochs, we perform the following experiment. 
\subsubsection{The accuracy drop analysis}
...
\subsubsection{Synthetic Graph with Domain Shift}
\label{sec:synthetic_graph}
...
\textbf{Data Generation:} The synthetic graph dataset consists of 500 graphs generated using PyTorch Geometric's FakeDataset, with an 80/20 train/test split applied once at initialization to prevent data leakage. Each graph contains an average of 15 nodes with 10-dimensional node features and belongs to one of 5 classes for graph-level classification. To simulate domain shift in the continual learning setting, we employ a task-shift paradigm where all tasks share the same 5 classes but introduce progressive perturbations across the 10-task sequence. Specifically, for task $k$ ($k \geq 1$), we apply three types of perturbations using a linear scaling mode: (1) Gaussian feature noise with standard deviation $\sigma_k = 0.02k$, (2) random edge dropout with probability $p_k = 0.01k$, and (3) constant feature shift of magnitude $s_k = 0.01k$. Task 0 serves as the unperturbed baseline. This design creates a challenging continual learning benchmark where the underlying classification task remains constant, but the input distribution shifts progressively---testing the model's ability to adapt to domain drift while retaining knowledge from earlier distributions.

\textbf{Model Architecture and Hyperparameters:} We employ a Graph Convolutional Network (GCN) architecture consisting of two graph convolution layers with hidden dimensions $[10 \rightarrow 32 \rightarrow 32]$, followed by max pooling and a 3-layer feedforward classifier with dimensions $[32 \rightarrow 32 \rightarrow 16 \rightarrow 5]$. All layers use LeakyReLU activations except the final classification layer. The model is trained using the Adam optimizer with learning rate $\eta = 10^{-4}$, batch size 128, and gradient clipping at norm 1.0. The Hamiltonian gradient is computed with weights $[\alpha, \beta, \gamma] = [0.4, 0.4, 0.1]$ for current task loss, experience replay loss, and regularization respectively. Each task is trained for 200 epochs with task warmup enabled (20 epochs at 10\% learning rate). For the AWB Full condition (Condition 4), architecture search is performed with the following settings: 5 preliminary training epochs, 150 A/B matrix training epochs, 50 warmup epochs, and modified Adam betas $(0.5, 0.999)$ for V-training to enable faster adaptation after the warmup-to-main transition.
...
\textbf{Interpretation:} The results reveal a favorable accuracy-forgetting trade-off for the AWB approach. The Baseline model exhibits \emph{flat accuracy} (74--81\%, range=7\%) across all domain shifts---it neither learns effectively from new domains nor forgets significantly. In contrast, AWB Full achieves \textbf{12.5\% higher average accuracy} (89.3\% vs 79.4\%) while maintaining a \emph{tighter accuracy range} (5\% vs 7\%), demonstrating consistent high performance despite increasing perturbation.

The forgetting analysis (Table~\ref{tab:forgetting_stats}) shows that AWB Full exhibits marginal forgetting (3.6\% average) on 8 out of 10 tasks, compared to near-zero forgetting for Baseline (0.5\% average on 2 tasks). However, this small forgetting cost is offset by dramatically superior learning capacity: AWB Full starts at higher accuracy and remains above Baseline even after forgetting. This demonstrates that the low-rank transfer mechanism ($V = A \cdot W \cdot B^T$) successfully preserves learned representations while enabling adaptation to new domains.
