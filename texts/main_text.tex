...
\subsubsection{Experiment 1}
 For the first experiment, we learn two tasks and train $500$ epochs on each task. 
 
 
 
 We  record in Figure \ref{reg5taskstrain} the loss function progression with respect to training epochs. In this experiment we first examine the behavior of a standard CL regime where no change in the architecture is performed (Condition C1). This serves as our baseline and is depicted by the red graph. Next, the blue graph describes the loss value if we change the architecture at each task with random re-initialization (Condition C3). The green graph shows the loss value when our method of changing the architecture is used at each step low-rank transfer is utilized to retain learning (Condition C4).
...
Figure~\ref{fig:sine_noise_comprehensive} provides a comprehensive comparison across all four experimental conditions (C1--C4) on the noisy sine regression benchmark with 3 random seeds. Panel (a) shows test MSE on experience replay data, with C4 (AWB Full) achieving consistently lower error throughout training. Panel (b) displays Hamiltonian loss convergence on a logarithmic scale, demonstrating faster and more stable convergence for C4. Panel (c) shows gradient norm evolution, indicating training stability across conditions. Panels (d)--(f) summarize the continual learning metrics: C4 achieves the lowest average MSE ($0.0249 \pm 0.0001$) compared with the baseline C1 ($0.0259 \pm 0.0000$), representing a 4\% improvement. The backward transfer (BWT) metric confirms that C4 maintains positive transfer while minimizing forgetting. These results demonstrate that the AWB transfer mechanism remains effective even in the presence of noisy data.
...
\textbf{Architecture Evolution:} Figure~\ref{fig:arch_evolution} visualizes the Hamiltonian loss trajectory alongside architecture changes at task boundaries for C3 (architecture search only) and C4 (AWB full). Each label indicates the network architecture at that task (e.g., $[64,64]$ denotes two hidden layers with 64 neurons each). In C3, architecture changes occur without knowledge transfer, resulting in loss spikes at each transition. In C4, the AWB transfer mechanism preserves learned representations, leading to smoother transitions and consistently lower Hamiltonian loss. This visualization provides direct evidence that architecture adaptation combined with low-rank transfer (C4) outperforms architecture adaptation alone (C3).
