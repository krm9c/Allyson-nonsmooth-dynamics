...
% Graph Convolutional Networks (GCN) consist of graph convolutional layers followed by feed-forward layers for graph classification.  In this section, we complete an explicit example that determines the sizes for $A$ and $B$ matrices for each layer of a convolutional neural network to transfer learning. This includes the $A$ and $B$ matrices for both the filter and the feedforward portion of the neural network.
...
% After completing an architecture search, we conclude the optimal architecture is:
